...
/** Bottom tabs: Dashboard → Activity → remaining modules; hub routes live under Activity. */
export function prepareBottomTabModules(modules: NavModule[], portal: NavPortalKind): NavModule[] {
  const sideFiltered = modules.filter(
    (m) => !SIDE_MENU_ROUTE_PATHS.has(m.routePath) && m.code !== 'MEMBER_PROFILE'
  );
  const tabFiltered = sideFiltered.filter((m) => !ACTIVITY_HUB_ROUTE_PATHS.has(m.routePath));
  ...
  return sortNavModules(tabFiltered);
}
